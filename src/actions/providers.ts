...
export async function submitProviderAction(
  ...
  if (!pool) {
    return {
      message:
        "Database is not configured. Please set your POSTGRES_URL environment variable.",
      success: false,
    };
  }
  ...
  try {
    if (id) {
      ...
      const ownerCheck = await client.query(
        "SELECT submitted_by_user_id FROM providers WHERE id = $1",
        [id]
      );
      if (
        ownerCheck.rows.length === 0 ||
        ownerCheck.rows[0].submitted_by_user_id !== submittedByUserId
      ) {
        return {
          message: "Authorization Error: You cannot edit this provider.",
          success: false,
        };
      }
      ...
      const photoUrl = imageUrl || `https://placehold.co/300x300.png`;
