...
export default function NewProviderPage() {
  ...
  useEffect(() => {
    if (!loading && !user) {
      router.push('/login?redirect=/providers/new');
    }
  }, [user, loading, router]);
  ...
  if (!user) {
    ...
    return (
      <PageWrapper className="flex flex-col items-center justify-center min-h-[calc(100vh-theme(spacing.32))] text-center">
        ...
        <p className="text-muted-foreground mb-4">You must be logged in to add a new doctor.</p>
        <Button asChild><Link href="/login?redirect=/providers/new">Login</Link></Button>
      </PageWrapper>
    );
  }
